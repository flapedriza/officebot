...
class OfficeAttendancePlugin(Plugin):
    ...
    @listen_to(r'^I will (not )?([a-z ]+) tomorrow$', re.IGNORECASE, direct_only=True)
    async def tomorrow_action(self, message: Message, not_match: Optional[str], action: str) -> None:
        ...
        action_date_object = date.today() + timedelta(days=1)
    ...
    @listen_to(
        r'^Who will ([a-z ]+) tomorrow\??$',
        re.IGNORECASE,
        needs_mention=True
    )
    async def list_people_action_tomorrow(self, message: Message, action: str) -> None:
        action_conversion = {
